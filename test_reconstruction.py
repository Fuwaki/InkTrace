...
def visualize_reconstruction(img, pred_maps, vector_paths, save_path, strokes=None, gt_targets=None):
    """
    可视化重建结果
    ...
    for i, curve in enumerate(vector_paths):
        ...
        axes[2, 0].plot(points[:, 0], points[:, 1], '-', color=curve_colors[i], linewidth=2)
        # Mark control points
        cp = np.array(curve['points'])
        axes[2, 0].plot(cp[:, 0], cp[:, 1], 'o', color=curve_colors[i], markersize=3, alpha=0.5)
    ...
    for i, curve in enumerate(vector_paths):
        points = bezier_to_points(curve['points'])
        axes[2, 1].plot(points[:, 0], points[:, 1], '-', color='red', linewidth=1, alpha=0.8)
    axes[2, 1].set_title('Curves on Input')
    axes[2, 1].axis('off')
    
    # Statistics
    if len(vector_paths) > 0:
        lengths = [np.linalg.norm(np.array(c['points'][3]) - np.array(c['points'][0])) for c in vector_paths]
        widths = [c['width'] for c in vector_paths]
        axes[2, 2].bar(['Curves', 'Avg Len', 'Avg Width'], [len(vector_paths), np.mean(lengths), np.mean(widths)])
    ...
    if strokes is not None and n_rows >= 4:
        ...
        if len(strokes) > 0:
            axes[3, 1].hist([len(s['points']) for s in strokes], bins=min(20, len(strokes)))
...
def test_single_sample(model, dataloader, device, output_dir, idx=0):
    ...
    for i, curve in enumerate(bezier_curves[:5]):
        ...
        print(f"    Curve {i}: width={curve['width']:.2f}, length={length:.1f}, "
              f"endpoints={p0} -> {p3}")
    ...
    visualize_reconstruction(img_np, pred_maps, bezier_curves, save_path, strokes, gt_targets)
