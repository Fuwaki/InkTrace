...
def visualize_detr_predictions(model, dataset, device, num_samples=6):
    ...
    with torch.no_grad():
        for i in range(num_samples):
            ...
            draw_bezier_strokes(
                axes[0, i], gt_strokes, gt_validity, threshold=0.5, color="lime"
            )
...
def compute_metrics(model, dataset, device, num_samples=100):
    ...
    with torch.no_grad():
        for i in range(min(num_samples, len(dataset))):
            ...
            if num_gt > 0 and num_pred > 0:
                ...
                coord_error = (
                    torch.abs(gt_curves[:min_curves, :8] - pred_curves[:min_curves, :8])
                    .mean()
                    .item()
                )
    ...
    if coord_errors:
        print(
            f"坐标误差 (L1): {np.mean(coord_errors):.6f} ± {np.std(coord_errors):.6f}"
        )
        print(
            f"宽度误差 (L1): {np.mean(width_errors):.6f} ± {np.std(width_errors):.6f}"
        )
...
def test_single_sample(model, dataset, device, index=0):
    ...
    with torch.no_grad():
        ...
        pred_strokes_np = strokes.cpu().squeeze(0).numpy()
        pred_validity_np = (
            validity.cpu().squeeze(0).squeeze(-1).numpy()
        )  # 移除最后的一维
    ...
    draw_bezier_strokes(
        axes[2], pred_strokes_np, pred_validity_np, threshold=0.5, color="red"
    )
