...
**Version**: 1.0
**Status**: Draft
**Scope**: Post-processing (CPU/Rust) logic to convert Dense Prediction Maps into Vector Paths.
...
*   **Tangent Map ($T$)**: $H \times W \times 2$, $(\cos 2\theta, \sin 2\theta)$. 描述骨架的切线方向（消除方向模糊）。
*   **Offset Map ($O$)**: $H \times W \times 2$, $(\delta x, \delta y)$. 描述像素中心到真实骨架中心的亚像素偏移。
